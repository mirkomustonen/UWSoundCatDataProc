...
plot_corr <- function(tob_data, pl_title) {
        ...
        names(tob_data) <- gsub('tob_', '', names(tob_data))
        names(tob_data) <- gsub('sdl_', '', names(tob_data))
        ...
        data_cor = cor(tob_data_comp, method = c("pearson"))
...
#' Make and save correlation plot from the input dataframe
#'
#' The corr_plot function makes a correlation plot from the inputted
#' dataframe and saves it into specified folder with specified file name.
#' 
